    ...
    public function register($req, $res) {
        ...
        if (!R::findOne('groups')) {
            ...
            foreach (['Superuser', 'Manager', 'Registered'] as $value) {
                $usergroupsTable = R::dispense('groups');
        ...
        if (!$req->files('avatar')->show('error')) {
            $avatar = $req->files('avatar')->upload(dirname(APPROOT) . '/public/assets/images');
            ...
            $body['avatar'] = explode('images/', $avatar)[1];
        } else {
            unset($body['avatar']);
        }
        ...
        if ($req->getMethod() == 'get') {
            ...
            if ($verification) {
                ...
                if ($user) {
                    ...
                    setFlashData('message', 'Password has been changed successfully.');
                    return $res->redirect(baseUrl('users/login'));
                } else {
                    
                    setFlashData('error', 'Wrong validation key passed.');
